...
VS20XX_VERSIONS = ["VS2012", "VS2015", "VS2017", "VS2019", "VS2022", "VS2026"]
...
def cpu_host_arch():
    ...
    if (
        ...
        .strip()
        .lower()
        .startswith(("amd64", "intel64", "em64t", "x86_64", "x86-64"))
    ):
        host = "x64"
        ...
        raise RuntimeError(
            f"Unsupported CPU architecture: {platform.machine()} ({platform.processor()})"
        )

    return host


def _get_vswhere_install() -> (
    list[dict[Literal["Version", "Edition", "Install"], str]] | None
):
    vswhere = Path(
    ...
    if vswhere.exists():
        ...
        print("Not found vswhere program.")
        return []
...
def _vc_ver_parser(vc_ver: str | VersionNum):
    if VERSION(vc_ver, ">=", "14.50"):
        ...
        raise Exception("WEMI doesn't support MSVC v140 and lower versions.")
