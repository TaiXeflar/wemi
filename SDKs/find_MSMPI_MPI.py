...
class FindMSMPI(FindSDK):
    ...
    def __WINDOWS__(self):
        ...
        msmpi_regedit_root = (
            ...
            if (
                raw_path := regedit(
                    "HKLM", r"SOFTWARE\Microsoft\MPI", key_name="InstallRoot"
                )
            )
            else None
        )
        ...
        try:
            mpiexec_dir_list = [
                ...
                for mpiexec in subprocess.run(
                    [self.es, r"mpiexec.exe"],
                    capture_output=True,
                    check=True,
                    text=True,
                ).stdout.splitlines()
            ...
            msmpi_mpiexec = [
                d
                for d in mpiexec_dir_list
                if d == (msmpi_regedit_root / "Bin/mpiexec.exe")
            ]
