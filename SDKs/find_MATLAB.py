...
class FindMATLAB(FindSDK):
    ...
    def __WINDOWS__(self):
        ...
        matlab_versioning = [self._matlab_version_analyzer(ver) for ver in matlab_versions]
        ...
        self.add_rule([
            ModulesObject(
                ...
                Version=ver,
                root=root.as_posix(),
                PATH=[Path(b).resolve().as_posix().replace(root.as_posix(), "$root/")])
                for ver, root, b in zip(matlab_versioning, matlab_install_dirs, matlab_binary_dirs)
        ])
