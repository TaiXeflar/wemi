...
from .refs._findVS20XX import (
    VC_ARCHS,
    cpu_host_arch,
    _get_vswhere_install,
    _vc_ver_parser,
)
    ...
    def __WINDOWS__(self):
        ...
        for vs in vs_install:
            ...
            for vc in vc_compilers_versions:
                vc_archs_installed = [
                    arch for arch in VC_ARCHS if (vc_root / vc / "lib" / arch).exists()
                ]
                ...
                self.add_rule(
                    [
                        ModulesObject(
                            ...
                            LIB=[
                                f"$root/lib/{arch}",
                                f"$root/ATLMFC/lib/{arch}"
                                if arch in ("arm64", "arm64ec")
                                else f"$root/ATLMFC/lib/{arch}",
                            ],
                            MODULEPATH=[
                                f".deps/winsdk/{arch}/",
                            ],
            ...
            if (ll_root / "bin/clang.exe").exists():
                llvm_version = self._find_version(
                    (ll_root / "bin/clang.exe").resolve(), "--version", "X.Y.Z"
                )
            ...
            if vs_cmake_exe.exists():
                vs_cmake_version = self._find_version(
                    vs_cmake_exe, "--version", "X.Y.Z"
                )
            ...
            vs_ninja_exe = Path(
                vs_dir / r"Common7/IDE/CommonExtensions/Microsoft/CMake/Ninja/ninja.exe"
            ).resolve()
            if vs_ninja_exe.exists():
                vs_ninja_version = self._find_version(
                    vs_ninja_exe, "--version", "X.Y.Z"
                )
                ...
                (
                    self.add_rule(
                        Module=f"ninja/{vs_ninja_version}",
            ...
            if msbuild_exe.exists():
                msbuild_ver = self._find_version(
