...
from .refs._findMHY import MIHOYO_PROJECTS_TYPEHINT, mihoyo_app_registry_phonebook, mihoyo_app_include_file_dict
        ...
        for k, v in mihoyo_app_registry_phonebook.items():
            ...
            if install_dir is None:
                ...
                with open(version_file, "r", encoding="utf-8") as f:
                    ...
                    version_info = re.search(r"version=(\d+\.\d+\.\d+)", config_content).group(1)
                    major, minor, _ = map(int, version_info.split("."))
                    version = f"{major}.{minor}" + f" (Luna-{minor+1})" if major == 6 and k == "GenshinImpact" else f"{major}.{minor}"
