    ...
    def __WINDOWS__(self):
        ...
        for cjc in cjc_list:
            ...
            cangjie_llvm_ver = self._find_version(
                cjc_dir / "third_party/llvm/bin/llvm-ar.exe", "--version", "X.Y.Z"
            )
            cangjie_llvm_tgt = (
                subprocess.run(
                    [(cjc_dir / "third_party/llvm/bin/llc.exe"), "--version"],
                    capture_output=True,
                    text=True,
                    check=True,
                )
                .stdout.splitlines()[3]
                .strip()
                .split("Default target: ")[-1]
            )
            cangjie_mingw_ld = cjc_dir / "third_party/mingw/bin/ld.exe"
            if cangjie_mingw_ld.exists():
                cangjie_mingw_ver = self._find_version(
                    cjc_dir / "third_party/mingw/bin/ld.exe", "--version", "X.Y"
                )
            else:
