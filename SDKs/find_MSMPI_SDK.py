...
class FindMSMPISDK(FindSDK):
    ...
    def __WINDOWS__(self):
        mpi_header_all = [
            Path(h).resolve()
            for h in self.everything(regex=r"^mpi\.h$")
            if h
            and not Path(h).resolve().parent.parent.joinpath("env", "vars.bat").exists()
        ]
        ...
        self.update(
            ...
            info_dict={"Install Dir": msmpi_dir.resolve().as_posix()},
        )
