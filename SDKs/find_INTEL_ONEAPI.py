...
from utils import regedit, subdirs, message, config
...
class FindOneAPI(FindSDK):
    ...
    def __WINDOWS__(self):
        ...
        self.add_rule(ModulesObject(
            Module=f'intel/oneapi',
            output=f'intel/oneapi',
            mode='tcl',
            Include_file='template_intel_oneapi',
            module_whaits=f'Intel oneAPI',
            root=self.ONEAPI_ROOT,
            ENVs={
                "ONEAPI_ROOT":  "$root"
            },
            MODULEPATH=[f'.deps/intel/oneapi']
        ))
    ...
    def add_intel_tbb(self, pth:Path) -> list[ModulesObject]: 
        ...
        for tbb in (pth).iterdir():
            ...
            tbb_list.extend([
                ModulesObject(
                    Module=f"intel/tbb/{tbb_ver} ({tbb_dist})" if tbb_dist else f"intel/tbb/{tbb_ver}",
                    output=f".deps/intel/oneapi/" + (f"intel/tbb/{tbb_ver}.{tbb_dist}" if tbb_dist else f"intel/tbb/{tbb_ver}"),
                    mode="tcl",
                    ...
                    PATH=[
                        '$tbb_root/bin' + f"/{tbb_dist}" if tbb_tree_new else f'$tbb_root/redist/intel64/{tbb_dist}'
                    ],
        ...
        for tcm in subdirs(pth):
            tcm_list.append(
                ModulesObject(
                    ...
                    output=f".deps/intel/oneapi/" + f"intel/tcm/{tcm.name}",
                    mode="tcl",
                    Include_file="template_intel_tcm",
                    VARs={'root':                   tcm,
                        'intel_target_arch':      INTEL_TARGET_ARCH,
                        },
                    ENVs={'ONEAPI_ROOT':            self.ONEAPI_ROOT,
                        'INTEL_TARGET_ARCH':      '$intel_target_arch',},
                    PATH=["$root/bin"]
            ))
        ...
        for mpi in mpi_vers:
            ...
            if (mpi/'bin/impi.dll').exists():
                self.add_rule(ModulesObject(
                    ...
                    output=f".deps/intel/oneapi/" + f'intel/mpi/{mpi.name}',
                    mode='tcl',
                    Include_file='template_intel_mpi',
                    Version=mpi.name,
                    conflicts=["Microsoft/msmpi/mpiexec", "Microsoft/msmpi/mpisdk", "intel/mpi"],
                    VARs={
                        "root": mpi.resolve().as_posix()
                    },
                    ENVs={
                        "ONEAPI_ROOT": self.ONEAPI_ROOT.as_posix()
                    },
                ...
                self.add_rule(ModulesObject(
                    ...
                    output=f".deps/intel/oneapi/" + f'intel/mpi/{mpi.name}.debug',
                    mode='tcl',
                    Include_file='template_intel_mpi',
                    Version=mpi.name,
                    conflicts=["Microsoft/msmpi/mpiexec", "Microsoft/msmpi/mpisdk", "intel/mpi"],
                    ...
                    PATH=['$root/bin', '$root/bin/mpi/debug', '$root/opt/mpi/libfabric/bin'],
                    INCLUDE=['$root/include'],
                ...
                self.add_rule(ModulesObject(
                    ...
                    PATH=['$root/bin', '$root/bin/release','$root/opt/mpi/libfabric/bin'],
                    INCLUDE=['$root/include'],
                ...
                self.add_rule(ModulesObject(
                    ...
                    PATH=['$root/bin', '$root/bin/debug', '$root/opt/mpi/libfabric/bin'],
                    INCLUDE=['$root/include'],
    ...
    def add_intel_compiler(self, pth:Path) -> list[ModulesObject]:
        ...
        message(f' -- Checking for Intel C/C++/DPC++/Visual Fortran HPC compilers')
        ...
        for ver in compiler_versions:
            ...
            if 'windows' in ver_install_items:
                ...
                icl = intel_compiler_version_grepper(ver/'windows/bin/intel64/icl.exe')
                icx = intel_compiler_version_grepper(ver/'windows/bin/icx.exe')
                ifort = intel_compiler_version_grepper(ver/'windows/bin/intel64/ifort.exe')
                ...
                llvm = self._find_version(ver/'windows/bin-llvm/clang.exe', '--version', 'X.Y.Z')
                ...
                compilers.append(ModulesObject(
                    ...
                    output=f".deps/intel/oneapi/" + f'intel/compiler/{ver.name}',
                    mode='tcl',
                ...
                compilers.append(ModulesObject(
                    ...
                    output=f".deps/intel/oneapi/" + f'intel/compiler/{ver.name}.llvm',
                    mode='tcl',
                    Include_file='template_intel_compiler',
                    Version=ver.name,
                    conflicts=['intel/compiler'],
                    ...
                    PATH=['$compiler_root/windows/bin',                                         # $compiler_root/windows/bin
                    ...
                    CPATH=['$compiler_root/windows/include',
                           '$compiler_root/windows/compiler/include',
                           '$compiler_root/windows/compiler/include/$intel_target_arch'
                          ],
                ...
                ifx = self._find_version(ver/'bin/ifx.exe', '--version', 'X.Y.Z', pattern=r"Version\s+([\d\.]+)")
                icpx = self._find_version(ver/'bin/icpx.exe', '--version', 'X.Y.Z')

                llvm = self._find_version(ver/'bin/compiler/clang.exe', '--version', 'X.Y.Z')
    ...
    def add_intel_mkl(self, pth:Path) -> list[ModulesObject]:
        ...
        message(f' -- Checking for Intel Math Kernel Library (MKL)')
        mkl_ver_dirs = subdirs(pth)

        for mkl in mkl_ver_dirs:
            mkl_ver = mkl.name

            libdirs = ['$root/lib', '$root/lib/intel64'] if (mkl/'lib/intel64').exists() else ['$root/lib']
            ...
            self.add_rule(ModulesObject(
                ...
                output=f".deps/intel/oneapi/" + f"intel/mkl/{mkl_ver}",
                mode='tcl',
                ...
                PATH=['$root/bin/intel64',
                      '$root/redist/intel64'
                     ],
                INCLUDE=['$root/include',
                         '$root/include/intel64/lp64'
                        ],
                LIB=libdirs,
            ...
            self.add_rule(ModulesObject(
                ...
                output=f".deps/intel/oneapi/" + f"intel/mkl/{mkl_ver}_ilp64",
                mode='tcl',
                Include_file="template_intel_mkl",
                Version=mkl_ver,
                conflicts="intel/mkl",
    ...
    def add_intel_dnnl(self, pth:Path) -> list[ModulesObject]: 
        ...
        for dnnl in subdirs(pth):
            ...
            if "bin" in dnnl_subdir:
                ...
                dnnl_list.append(ModulesObject(
                    ...
                    PATH=[
                        f'$root/$dnnl_version/bin',
                    ],
                    INCLUDE=[
                        f'$root/$dnnl_version/include',
                    ],
                    LIB=[
                        f'$root/$dnnl_version/lib',
                    ],
                    CMAKE_PREFIX_PATH=[
                        f'$root/$dnnl_version/lib/cmake',
                    ],
                    PKG_CONFIG_PATH=[
                        f'$root/$dnnl_version/lib/pkgconfig',
                    ]
                ))
                ...
                dnnl_list.extend([ModulesObject(
                    Module=f"intel/dnnl/{dnnl_ver}.{b}",
                    output=f".deps/intel/oneapi/" + f"intel/dnnl/{dnnl_ver}.{b}",
                    mode='tcl',
                    Include_file="template_intel_dnnl",
    ...
    def add_intel_gdb(self, pth:Path) -> list[ModulesObject]: 
        ...
        for gdb in subdirs(pth):
            ...
            self.add_rule(ModulesObject(
                ...
                output=f".deps/intel/oneapi/" + f'intel/gdb/{gdb.name}',
                mode='tcl',
                Include_file='template_intel_gdb',
                conflicts=['intel/gdb'],
                VARs={
                    "root": gdb.resolve().as_posix()
                },
                ENVs={
                    "ONEAPI_ROOT": self.ONEAPI_ROOT.resolve().as_posix()
                },
