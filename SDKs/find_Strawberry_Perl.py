...
class FindStrawberryPerl(FindSDK):
    ...
    def __WINDOWS__(self):
        ...
        for perl in perl_lst:
            ...
            perl_gcc =      self._find_version(perl.parent.parent.parent/"c/bin/gcc.exe", "--version", "X.Y.Z")
            perl_gcct =     subprocess.run([perl.parent.parent.parent/"c/bin/gcc.exe", "-dumpmachine"], text=True, capture_output=True, check=True).stdout.strip()
            perl_cmake =    self._find_version(perl.parent.parent.parent/"c/bin/cmake.exe", "--version", "X.Y.Z")
            perl_ninja =    self._find_version(perl.parent.parent.parent/"c/bin/ninja.exe", "--version", "X.Y.Z")
            ...
            self.add_rule(ModulesObject(
                ...
                PATH=["$root/perl/bin", "$root/perl/site/bin", "$root/c/bin", "$root/c/x86_64-w64-mingw32/bin"],
