...
class WindowsCheck:
    ...
    def __init__(self): 
        ...
        self.check_python_interps(allow_interps=['CPython', ])
        self.check_python_environ()
        self.check_python_winreg()
        self.check_everything_service()
        self.check_everything_cli()

    def check_python_version(self, tgt:VersionNum, ver:VersionNum|None=..., MAX:VersionNum|None=..., /):
        msg = f' -- Checking for Python version compatiable'
        message(msg)
        wait(0.3)
        if ver and not MAX:
            _v = VERSION(ver, '<=', tgt)
    ...
    def check_python_environ(self):
        msg = f' -- Checking for Python executable environment type'
        message(msg)
        wait(0.3)
    ...
    def check_everything_cli(self):
        ...
        if es1.exists():
            ...
            message(f'{msg:<74} -- Failed')
            msg = f' -- Checking for WEMI .deps/es.exe exists\t'
            message(msg)
            if es2.exists():
                message(f'{msg:<74} -- Found')
            else:
